...
outputfront_dir = my_params.output_dir+'\\lms_front_'+ my_params.dataset_no + '\\'
outputrear_dir  = my_params.output_dir+'\\lms_rear_'+ my_params.dataset_no + '\\'
...
lms_front_timestamps = []
with open(lms_front_timestamps_path) as lms_front_timestamps_file:
    num_of_laser = 0
    for _,row in enumerate(lms_front_timestamps_file):
        lms_front_timestamp = int(row.split(' ')[0])
        
        if (lms_front_timestamp > end_time): 
            print("num_of_laser_scan: ",num_of_laser)
            break

        requested_timestamps.append(lms_front_timestamp)
        lms_front_timestamps.append(lms_front_timestamp)
        num_of_laser += 1
lms_front_timestamps_file.close()
...
for i in range(num_of_laser):
    # if i > 500: break
    if lms_front_timestamps[i] >= end_time: break
    ...
    file_path = os.path.join(lms_front_folder_path + '\\' + str(lms_front_timestamps[i]) + '.bin')
    ...
    np.savetxt((outputfront_dir + str(lms_front_timestamps[i]) + '.csv'), save_csv.transpose(), delimiter=",")
