...
lms_folder_path = my_params.dataset_patch + 'lms_front'
lms_timestamps_path = my_params.dataset_patch + 'lms_front.timestamps'
...
lms_timestamps = []
with open(lms_timestamps_path) as lms_timestamps_file:
    num_of_laser = 0
    for _,row in enumerate(lms_timestamps_file):
        lms_timestamp = int(row.split(' ')[0])
        
        if (lms_timestamp > end_time): 
            print("num_of_laser_scan: ",num_of_laser)
            break

        requested_timestamps.append(lms_timestamp)
        lms_timestamps.append(lms_timestamp)
        num_of_laser += 1
lms_timestamps_file.close()
...
for i in range(num_of_laser):
    if i > 500: break
    if lms_timestamps[i] >= end_time: break
    ...
    file_path = os.path.join(lms_folder_path + '\\' + str(lms_timestamps[i]) + '.bin')
