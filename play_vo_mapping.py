...
with open(vo_directory) as vo_file:
    ...
    for row in vo_reader:
        ...
        # TEST_data = [50,50,40,0,0,0]
        # TEST_pose = build_se3_transform(TEST_data)

        # TEST_pose=H_new@TEST_pose    # wolrd-cord transform
        # x_TEST_pose=int(x_map + TEST_pose[1,3])
        # y_TEST_pose=int(y_map + TEST_pose[0,3])
        # vo_map = cv2.circle(vo_map,(x_TEST_pose,y_TEST_pose),5,(255,0,0),thickness=1)
            ...
            for _,row in enumerate(lidar_timestamps_file):
                ...
                if(draw_flag == 1):
                    ...
                    for i in range(objs_data.shape[1]):
                        ...
                        x_obj=int(x_map + obj_pose[1,3])
                        y_obj=int(x_map + obj_pose[0,3])
